 ...
 *
 * ★★ **この関数は画面から外してある（2026-08-18）。** ★★
 *   実データで検証したところ、**対象ユーザーに対して既定設定では no-op** だった。
 *
 *   窓 ≧ (所持ライボ ÷ 焚き数) × 最長曲の cycleSec を満たすと、
 *   **全曲の plays が同じ値になり、totalPt = plays × eventPt なので並びが Pt/回 と完全一致する**。
 *   イベラン層の 10炊き・ライボ50 では境界がわずか18分で、それを超えたら
 *   2時間だろうと24時間だろうと通常のオートタブと1位から最下位まで同じ順位になる。
 *   焚き数を振った24通りの総当たりでも、10炊きでは差が出たケースが 0/24 だった。
 *
 *   差が出るのは refill=ON かつ窓が「99回を流し切るには足りないが大きくは足りている」帯
 *  （走者条件で 3.5〜5.9時間）だけ。しかもそこでは**石の消費**が判断に要るのに、
 *   refill を taki=0 で近似している現状ではコストが一切出ない。
 *   同じ時間を手動で回した方が Pt が1.67倍という比較すらできない。
 *
 *   ★ 復活させるなら、最低限これを直すこと:
 *     1. refill=ON のとき必要LB・石を出す（lbNeededForPlays が既にある）
 *     2. 窓モード専用のサマリに分岐する（今は無関係な「100回プラン」が出る）
 *     3. 「今日の残りオート回数」を窓モードでも表示する（順位を支配する隠れ入力）
 *     4. windowSec === 0 を「0回」として扱う（今は時間無制限になる）
 *     5. stoppedBy を実際に先に当たった制約で返す（今はライボ切れが「時間切れ」になる）
 *     6. 既定を refill=ON にするか、「注ぎ足さない限り窓は効かない」と画面に書く
 *
