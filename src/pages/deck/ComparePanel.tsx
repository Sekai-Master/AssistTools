 ...
 *
 * ★ スキル値は**編成ごとに計算している**（lib/skill.ts）。カードのスキルレベルから
 *   出せるようになったので、以前の「全編成共通の近似」は廃止した。
  ...
  return (
    <Panel title="編成を比べる">
      ...
      {/* ★ イベント未選択のときは 0% として並べない（「ボーナス0の編成」に見えてしまう）。
          ただしチャレンジライブは**そもそもイベントが無い**ので、この関門を通さない
          （通すと比較が一度も出せなくなる。実際にそうなっていた）。 */}
      {!challenge && ctx.eventId == null ? (
