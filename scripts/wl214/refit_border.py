...
使い方:
    ...
    python refit_border.py --at 20:00 # 指定時刻(JST)以前で最も新しいスナップショットでアンカー
...
def main():
    ...
    if "--at" in sys.argv:
        ...
        jst = datetime.datetime.now().replace(hour=h, minute=m, second=0, microsecond=0)
    ...
    for rk in RANKS:
        ...
        w = max(abs(e) for e in errs)
        # レンジは「参照倍率の min/max」と「LOO の最悪誤差」の両方を包む。
        # ⚠️片側にしか誤差が出ていないときに対称に広げると、観測の裏づけ無しに
        #   楽観側の余裕を捏造する（2026-08-25 の破壊レビューで実際に踏んだ）。
        lo = min(cur[rk] * ms[0], cur[rk] * med * (1 - w)) if min(errs) < 0 else cur[rk] * ms[0]
        hi = max(cur[rk] * ms[-1], cur[rk] * med * (1 + w)) if max(errs) > 0 else cur[rk] * ms[-1]
