...
> **2026-08-01 追記**: 上の3色は `--neu-base` / `--neu-lo` / `--neu-hi` というトークンになり、ダークテーマでは別の3値に解決される。**関係（左上光源・浮き沈みの造形・角丸）は不可侵のまま**で、変わるのは素材の色だけ。詳細は [../theming.md](../theming.md)。
>
> ホバー時に**影の暗い側だけ**をその要素の色に染める `.neu-lit` を追加した（Hub のカード）。光源は増やさない ── 色が付くのは「カードの色が落ちた影に回り込んでいる」ぶんだけで、ハイライト側は白のまま。**動く光（border-beam 等）は入れない**: 1つの固定光源という前提が壊れると素材が素材に見えなくなる。
...
- WxS: 天馬司 #FB0 / 鳳えむ #F6B / 草薝寧々 #3D9 / 神代類 #B8E
...
## 影を触るときの規約（2026-07-31 追加。遷移演出の導入に伴う）

影の量は `--neu-lift`（0 = 素材と面一 / 1 = 通常）という 1 変数に集約されており、
`@theme` の影トークン4本は `calc(Npx * var(--neu-lift, 1))` になっている。
値は `--neu-lift = 1` のとき上表と完全に等価。詳細は [../motion-system.md](../motion-system.md)。

以下は破ると静かに壊れるので守ること。

- **`--neu-lift` は `html`(`:root`) 上でだけ動かす。** 子孫要素に置いても
  `.neu-panel` などのコンポーネントクラスは追随しない（`var()` は宣言された要素で
  置換されるため、`:root` の時点で値が焼き付く）。追随するのは要素側で `--tw-shadow` を
  宣言する Tailwind ユーティリティだけなので、「パネルの影は据え置きで中のボタンだけ平ら」
  という真逆の絵になる。実ブラウザで検証済み。
- **遷移演出に `transform` を使わない。** `.stage` に `transform` が乗ると
  `position: fixed` の包含ブロックが切り替わり、ついぼのモバイル常設アクションバーや
  ポータルを使っていないページ内モーダルの位置が壊れる。`isolation: isolate` も同様
  （モーダルが sticky ヘッダーの下に潜る）。奥行きは影だけで語る。
- **遷移中も浮いていてほしい要素は影をリテラルで書く**（`.neu-static`）。
  無地の間はトークンが意図的に 0 へ潰れているので、トークンを使うと
  まさに見せたい場面で見えなくなる。
